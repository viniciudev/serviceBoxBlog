...
export default class MaryKay extends Component {
    state = {
        files: [],
    ...
    componentDidMount() {
        axios.get(`${URL_DescriptionFiles}/${enumGroup.maryKey}/group`).then(resp => {
            const { data } = resp
            if (data) {
    ...
    consultFilter(e) {
        ...
        if (e.key === 'Enter') {
            ...
            axios.get(`${URL_SearchDescriptionFiles}/${text}/${enumGroup.maryKey}`).then(resp => {
                const { data } = resp
                if (data)
                    this.setState({ formFiles: data })
            })
    ...
    query = (pageSizeValue, pageNumber) => {
        ...
        axios.patch(`${URL_DescriptionFiles}/${enumGroup.maryKey}/group`, data).then(resp => {
            const { data } = resp
            if (data)
                this.setState({
                    formFiles: data
                                    ...
                                    f.groupItems == 1 ?
                                        <Col md={3} >
                                            <Card>
                                                ...
                                                <a className="ml-1" href="">Mary Kay</a>
                                                            </small>
