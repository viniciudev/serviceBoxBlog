...
export default class Jewes extends Component {
    state = {
        files: [],
    ...
    componentDidMount() {
        axios.get(`${URL_DescriptionFiles}/${enumGroup.jewels}/group`).then(resp => {
            const { data } = resp
            if (data) {
    ...
    consultFilter(e) {
        ...
        if (e.key === 'Enter') {
            ...
            axios.get(`${URL_SearchDescriptionFiles}/${text}/${enumGroup.jewels}`).then(resp => {
                const { data } = resp
                if (data)
                    this.setState({ formFiles: data })
            })
    ...
    query = (pageSizeValue, pageNumber) => {
        ...
        axios.patch(`${URL_DescriptionFiles}/${enumGroup.jewels}/group`, data).then(resp => {
            const { data } = resp
            if (data)
                this.setState({
                    formFiles: data
